...
ColorGradingShader::ColorGradingShader(ID3D11Device* device)
{
	//頂点シェーダー
	{
		//ファイルを開く
		FILE* fp = nullptr;
		fopen_s(&fp, "Shader\\ColorGrandingVS.cso", "rb");
	...
	//ピクシェルシェーダー
	{
		//ファイルを開く
		FILE* fp = nullptr;
		fopen_s(&fp, "Shader\\ColorGrandingPS.cso", "rb");
		_ASSERT_EXPR_A(fp, "CSO File not found");
		...
		//メモリ上にピクセルシェーダーデータを格納を用意する
		std::unique_ptr<u_char[]> csoData = std::make_unique<u_char[]>(csoSize);
		fread(csoData.get(), csoSize, 1, fp);
		fclose(fp);

		//ピクシェルシェーダー生成
		HRESULT hr = device->CreatePixelShader(csoData.get(), csoSize, 
	...
	//定数バッファ
	{
		...
		desc.MiscFlags = 0;
		desc.ByteWidth = sizeof(CBColorGranding);
		desc.StructureByteStride = 0;

		HRESULT hr = device->CreateBuffer(&desc, 0, colorGrandingConstantBuffer.GetAddressOf());
		_ASSERT_EXPR(SUCCEEDED(hr), HRTrace(hr));
	}
...
{
	CBColorGranding cbColorGranding;
	cbColorGranding.hueShift = rc.colorGrandingData.hueShift;
	cbColorGranding.saturation = rc.colorGrandingData.saturation;
	cbColorGranding.brightness = rc.colorGrandingData.brightness;
	rc.deviceContext->UpdateSubresource(colorGrandingConstantBuffer.Get(), 0, 0, &cbColorGranding, 0, 0);
...
//描画終了
void ColorGradingShader::End(const RenderContext& rc)
{
